...
#              print "Could not use " + str(type(aCurAbility));
  ...
  def __init__(self,  economy,  gameMap,  army,  output,  msgBaseDelaySecs = 0.10):
    ...
    self.savedGameMap = copy.deepcopy(gameMap)
    # deep copy army
    self.saveArmy = copy.deepcopy(army)
...
#              output.log(pformat(vars(cCurAttacker)) + "\n");
  ...
  def BuildCurrentState(self):
    # Get gamemap
    L = ""
        
    # For each x
    for x in range(0,self.gameMap.xSize):
        # For each y
        for y in range(0,self.gameMap.ySize):
            if len(self.gameMap.getTraps(x,y)) > 0 :
                L += "1"
            else:
                L += "0"
                
            if len(self.gameMap.getTreasures(x,y)) > 0:
                L += "2"
            else:
                L += "0"
                
            if len(self.gameMap.getFoes(x,y)) > 0:
                L += "3"
            else:
                L += "0"
                
            # Also check for attackers here
            bAddedAttacker = false
            for attacker in self.aAttackers:
                if attacker.x == x && attacker.y == y:
                    L += "4" # We use the same symbol for all types of attackers at this point
                    bAddedAttacker = True
                    break
            if !bAddedAttacker:
                L += "0"
  ...
  def reset(self):
    self.gameMap = copy.deepcopy(self.savedGameMap)
    self.aAttackers = copy.deepcopy(self.savedArmy)


  # 0 means the agents lost. 1 means the agents (those you are still alive) won
  def attackersWon():
    if (len(self.aAttackers) > 0)
        return 1
    else:
...
if __name__ == "__main__":
  ...
  army = Game.selectArmy(economy,  gameMap,  sAttackerColor,  output, ['SoldierClass']);
  # Init game
